...
思路：先遍历 A 链表，将节点存到集合中，然后遍历 B 链表，找到存在的节点直接返回。
...
- 时间复杂度：O(m + n)，m、n 分别是 A 链表 和 B 链表 的长度。
- 空间复杂度：O(m)，m 是 A 链表的长度。



方法二：双指针

思路：双指针 pA、pB 分别指向 A 和 B 进行遍历，pA 指向 NULL 时就去指向 B 链表，pB 指向 NULL 时就去指向 A 链表。双指针指向同一个节点时就是相交节点，走过的路程：pA + pB = pB + pA。
